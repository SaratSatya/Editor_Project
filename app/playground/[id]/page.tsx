...
const MainPlaygroundPage = () => {
    ...
    const {setTemplateData,setActiveFileId,setPlaygroundId,setOpenFiles,activeFileId,closeAllFiles,closeFile,openFile,openFiles}=useFileExplorer()
    ...
    const activeFile=openFiles.find((file:any)=>file.id===activeFileId)
    const hasUnsavedChanges=openFiles.some((file:any)=>file.hasUnsavedChanges);
                        ...
                        {openFiles.map((file)=>(
                          <TabsTrigger key={file.id} value={file.id} className="relative h-8 px-3 data-[state=active]:bg-background data-[state=active]:shadow-sm group">
                            <div className='flex items-center gap-2'>
                              ...
                              <span className='m1-2 h-4 w-4 hover:bg-destructive hover:text-destructive-foreground rounded-sm flex items-center jusitfy-center opacity-0 group-hover:opacity-100 transition-opacity cursor-pointer' onClick={(e)=>{e.stopPropagation(); closeFile(file.id);}}>
                                <X className='h-3 w-3'/>
                              </span>
                            </div>
                          </TabsTrigger>
                ...
                <div className="flex-1">
                  <ResizablePanelGroup direction="horizontal" className="h-full">
                    <ResizablePanel defaultSize={isPreviewVisible?50:100}>
                      <PlaygroundEditor activeFile={activeFile} content={activeFile?.content || ""} onContentChange={()=>{}}/>
                    </ResizablePanel>
                  </ResizablePanelGroup>
                </div>
              </div>
