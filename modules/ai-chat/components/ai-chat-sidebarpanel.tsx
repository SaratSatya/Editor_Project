...
export const AIChatSidePanel: React.FC<AIChatSidePanelProps> = ({
  ...
  const getChatModePrompt = (mode: string, content: string) => {
    switch(mode){
      case "review":
        return `Please review this code and provide detailed suggestions for improvements, include performance, security, and best practices:\n\n**Request:**${content}`;
      case "fix":
        return `Please help fix isses in this code, including bugs,errors and potential problems:\n\n**Problem:**${content}`
      case "optimize":
  ...
  const handleSendMessage = async (e: React.FormEvent) => {
    ...
    try{
      ...
      const response = await fetch("/api/chat", {
          ...
          body: JSON.stringify({
              message: contextualMessage,   // FIXED
      ...
      if(response.ok){
        ...
        setMessages((prev)=>[
          ...
          {
            role:"assistant",
            content:"Sorry, I encounterd an error while processing your request. Please try again.",
            timestamp:new Date(),
            id:Date.now().toString(),
          },
        ]);
                    ...
                    >
                      ...
                      <div className="prose prose-invert prose-sm max-w-none">
                        <ReactMarkdown
                          ...
                          components={{
                            code: (props) => {
                              const { children, className, inline } = props as { children: React.ReactNode; className?: string; inline?: boolean };
                              if (inline) {
                                return (
